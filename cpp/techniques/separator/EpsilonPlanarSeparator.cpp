...
namespace {
double getComponentCost(const std::vector<NetworKit::node> &cc, std::vector<double> &vCost) {
    double c = 0.0;
    for (auto v : cc) {
        c += vCost[v];
    }
...
void processSide(std::vector<NetworKit::node> partition, std::vector<double> &vertexCost,
...
void EpsilonPlanarSeparator::run() {
    ...
    while (!Q.empty()) {
        ...
        auto mapFromInducedToOriginal =
            [&](const std::vector<NetworKit::node> &vec) -> std::vector<NetworKit::node> {
        ...
        processSide(mapFromInducedToOriginal(sepAlgo.getPartitionA()), vertexCost, idToComponentMap,
                    epsilon, Q, idCnt);

        processSide(mapFromInducedToOriginal(sepAlgo.getPartitionB()), vertexCost, idToComponentMap,
                    epsilon, Q, idCnt);
    }
