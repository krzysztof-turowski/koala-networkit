...
void read_edge(std::ifstream &graph_file, NetworKit::Graph &graph, const std::string &format) {
...
std::tuple<NetworKit::Graph, NetworKit::node, NetworKit::node> DimacsGraphReader::read_all(
    ...
    std::ifstream graph_file{std::string{path}};
    ...
    while (true) {
        ...
        switch (command) {
            ...
            case 'c':
                break;
            case 'a':
                read_edge(graph_file, graph, format);
                break;
            case 'e':
                if (graph.isDirected()) {
