...
template <typename Value>
LinkCutTree<Value>::Node* LinkCutTree<Value>::findRoot(Node *node) {
    access(node);
...
template <typename Value>
LinkCutTree<Value>::Node* LinkCutTree<Value>::findParent(Node *node) {
    access(node);
    if (node->left == nullptr) {
        return nullptr;
    }
...
template <typename Value>
LinkCutTree<Value>::Node* LinkCutTree<Value>::findRightmostAtMost(Node *node, Value limit) {
    push(node);
...
template <typename Value>
LinkCutTree<Value>::Node* LinkCutTree<Value>::get(NetworKit::node v) {
    return nodes[v].get();
}
...
NetworKit::Edge LinkCutTree<Value>::findSaturatedEdge(NetworKit::node v) {
    ...
    return parent == nullptr ? NetworKit::Edge() : NetworKit::Edge(bottleneck->id, parent->id);
}

template <typename Value>
std::optional<Value> LinkCutTree<Value>::getMinimumPathResidualCapacity(NetworKit::node v) {
    auto *node = get(v);
    access(node);
    return node->minimum;
}

template class LinkCutTree<int>;
template class LinkCutTree<double>;
