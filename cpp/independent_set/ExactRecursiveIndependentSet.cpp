...
std::vector<NetworKit::node> Mis2IndependentSet::recursive() {
    ...
    case 3:{
        ...
        if (!graph->hasEdge(u1, u2) && !graph->hasEdge(u2, u3) && !graph->hasEdge(u1, u3)) {
            ...
            if (!vMirrors.empty()) {
                ...
                std::vector<std::vector<NetworKit::node>> solutions{setCase1, setCase2, setCase3, setCase4};
    ...
    auto lastIndex = std::numeric_limits<NetworKit::node>::min();
