...
bool ExactIndependentSet::edgeComparator(const NetworKit::Edge& a, const NetworKit::Edge& b) {
    return a.u < b.u || (a.u == b.u && a.v < b.v);
}

std::vector<NetworKit::node> ExactIndependentSet::getNeighbors(NetworKit::node v) const {
    return std::vector<NetworKit::node>(
        graph->neighborRange(v).begin(), graph->neighborRange(v).end());
}

std::vector<NetworKit::node> ExactIndependentSet::getNeighborsPlus(NetworKit::node v) const {
    auto neighborsPlus = getNeighbors(v);
    neighborsPlus.push_back(v);
    return neighborsPlus;
}

std::vector<NetworKit::node> ExactIndependentSet::getNeighbors2(NetworKit::node v) const {
...
ExactIndependentSet::EdgeSet ExactIndependentSet::getConnectedEdges(
...
ExactIndependentSet::EdgeSet ExactIndependentSet::getInducedEdges(
        std::vector<NetworKit::node>& nodes) const {
    EdgeSet connectedEdges(edgeComparator);
...
std::vector<NetworKit::node> ExactIndependentSet::getMirrors(NetworKit::node v) const {
...
NetworKit::node ExactIndependentSet::getMinimumDegreeNode() const {
...
NetworKit::node ExactIndependentSet::getMaximumDegreeNode() const {
    assert(!graph->isEmpty());
    return *std::max_element(graph->nodeRange().begin(), graph->nodeRange().end(),
        [&](NetworKit::node v, NetworKit::node u) {return graph->degree(v) < graph->degree(u);});
}

void ExactIndependentSet::removeElements(std::vector<NetworKit::node> nodes) {
    for (auto v : nodes) {
        graph->removeNode(v);
    }
}

std::vector<NetworKit::node> ExactIndependentSet::runIndependentSetDegree2() const {
