...
void Koala::CographRecognition::Lcheck() {
...
void Koala::CographRecognition::Rcheck() {
...
void Koala::CographRecognition::AddElementToPart(part *P, element *v) {
    P->size++;
    v->my_part = P;
    if (P->first == nullptr) {
        P->first = v;
...
void Koala::CographRecognition::EraseElement(element *v) {
    element *A, *B;
    A = v->previous;
    B = v->next;
    v->my_part->size--;
...
void Koala::CographRecognition::AddPart(part *x, part *y) {
    part *A = x->next;
    x->next = y;
    y->previous = x;
    y->next = A;
...
void Koala::CographRecognition::ShowTheOrder() {
    part *x;
    element *y, *z;
    x = first_part;
    while (true) {
...
void Koala::CographRecognition::ErasePart(part *p) {
