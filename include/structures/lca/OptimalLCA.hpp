...
public:
    ...
    // query in O(1) time
    [[nodiscard]]
    NetworKit::node query(NetworKit::node u, NetworKit::node v) const;
...
protected:
    ...
    // Similar to B in the paper. In our implementation, it holds values incremented by `blockOffset`
    ...
    void EulerDFS(NetworKit::node u, NetworKit::count level);
...
template <class T>
void OptimalLCA<T>::EulerDFS(const NetworKit::node u, const depth_t level) {
...
template <class T>
void OptimalLCA<T>::populatePrecomputedRMQSmallBlocks() {
...
template <class T>
void OptimalLCA<T>::populateBlockArrays() {
...
template <class T>
void OptimalLCA<T>::populateSparseTable() {
