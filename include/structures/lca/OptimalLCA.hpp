...
class OptimalLCA {
 public:
    ...
    OptimalLCA(const std::vector<NetworKit::node> &parent, NetworKit::node root);
...
inline OptimalLCA::OptimalLCA(const std::vector<NetworKit::node> &parent, NetworKit::node root)
        : parent(parent), children(parent.size()), root(root) {
    for (NetworKit::node u = 0; u < parent.size(); u++) {
        if (parent[u] != NetworKit::none) {
            children[parent[u]].push_back(u);
        }
    }
    R.assign(parent.size(), NetworKit::none);
    euler_DFS(root, 1);
...
inline NetworKit::node OptimalLCA::query(NetworKit::node u, NetworKit::node v) const {
    if (block_size < MIN_ALLOWED_BLOCK_SIZE) {
...
inline void OptimalLCA::verify() const {
    ...
    assert(E.size() == 2 * parent.size() - 1);
}

inline void OptimalLCA::euler_DFS(const NetworKit::node u, const depth_t level) {
...
inline NetworKit::index OptimalLCA::query_small_block(
...
inline void OptimalLCA::populate_precomputed_RMQ_small_blocks() {
...
inline void OptimalLCA::populate_block_arrays() {
...
inline OptimalLCA::mask_t OptimalLCA::get_mask(
        std::vector<OptimalLCA::depth_t>::iterator begin,
        std::vector<OptimalLCA::depth_t>::iterator end) {
    mask_t mask = 0;
    std::advance(begin, 1);
...
inline void OptimalLCA::populate_sparse_table() {
...
inline NetworKit::node OptimalLCA::query_naive(NetworKit::node u, NetworKit::node v) const {
    std::unordered_set<NetworKit::node> visited;
    auto vertex = u;
    while (vertex != NetworKit::none) {
