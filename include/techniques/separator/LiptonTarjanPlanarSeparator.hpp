...
private:
    ...
    std::tuple<int, int, int> find_partition_levels(const NetworKit::Graph &G,
                                                    std::vector<NetworKit::node> &verticesAtLevel,
                                                    std::vector<int> &lvl);

    std::vector<double> find_subtree_costs(const NetworKit::Graph &H,
    ...
    void find_separator_from_components(std::vector<std::vector<NetworKit::node>> &components);
    void extract_separator_and_partition_from_cycle(const NetworKit::Graph &G,
                                                    const std::vector<int> &lvl, int l1, int l2,
                                                    std::vector<NetworKit::node> &finalCycle,
                                                    NetworKit::node x);
    double compute_middle_cost(const NetworKit::Graph &G, const std::vector<int> &lvl, int l1,
                               int l2);
};
