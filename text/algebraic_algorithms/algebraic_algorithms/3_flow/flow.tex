...
Showing a good lower bound $\delta$ is however not easy. First of all we do not know if target flow can be fully routed through $G$. If it is not the case, the algorithm terminates through $y$. Even if the demand can be satisfied, we are still yet to prove the algorithm reaches the desired flow.
...
Now we can construct an upper-bound on $\delta$ necessary to prove \Cref{lemma_step_bound}. From \Cref{step_upperbound} we have
\[
\delta
\ge \frac{1}{33||\rho||_4}
\ge \frac{1}{33||\rho||_2}
...
This, in turn, in conjunction with \Cref{time_complexity_flow} completes the proof that the time complexity of this algorithm is
