...
algorithm that runs in $O(n \log{n})$ time and which worst-case ratio is 1 -
$O(1/ \log{\log{n}})$. One can find it interesting why we cannot use Algorithm
$MISP$ directly and have worst-case ratio and running time guaranteed by Theorem
\ref{thm:MISP-main-theorem}. That is because the third property is not
satisfied. The counter example is graph $K_{1, n-1}$ which matching is 1
...
\begin{enumerate}
    ...
    \item $G$ has a vertex of degree 1. Let $u$ be the vertex adjacent to $v$ in
        ...
        the mathing. Also since $S^*(G)$ has exactly one edge adjacent to $u$
    ...
    \item $G$ has a vertex of degree 2. Let $u, w$ be vertices adjacent to $v$
        and $G'$ be a graph obtained from $G$ by indentifying those three
        ...
        correct mathing of size at most one less than the latter. In the other
...
\begin{enumerate}
    \item If $N \leq \log{\log{n}}$, so we reduced our graph to a small fraction
        of the original one, we apply an exact mathing algorithm, so
        $\frac{|S(G)|}{|S^*(G)|} = 1$ (if the denominator is 0, then we set this
        ratio to 1)
    \item If $N \geq \log{\log{n}}$ and $\forall_{v \in V(G)} \, deg(v) \geq 3$
        we apply $MISP$ algorithm. For it to correct we have to check conditions
        from Theorem \ref{thm:MISP-main-theorem}:
        \begin{enumerate}
            ...
            \item Let $S^*(G)$ be the maximum matching. By removing vertices of
                ...
                matching here. So $|S(G)| \geq S^*(G) - |C|$, by rearanging we
                have $|S^*(G)| - |S(G)| = |C| = O(|C|)$, so the error is bound
                by size of $C$.
            \item Here we will use the theorem from \cite{nishizeki1979lower}
                which guarantees that if $G$ is planar and has degree of at
                least 3, then $|S^*(G)| \geq N/3$, so the optimal solution is a
        ...
        Theorem \ref{thm:MISP-main-theorem}.
\end{enumerate}

\subsection{Time complexity}
At first we should discuss the time complexity of reductions. By keeping a graph
as adjecency lists, so that each vertex not only has a previous reference and
next preference but also the cross reference to other linked lists, we can
perform a deletion in $O(deg(v))$. And as each vertex is deleted only once, we
can do that in amortized time $O(1)$. Another operation is indentification of
vertices which can be performed in total time $O(n \log{n})$
\cite{chibaApplicationsLiptonTarjans1981}. To do that we keep an aditional
adjecency matrix and rewiring edges from the vertex of a smaller degree to one
...
\log{\log{n}}$, then from Theorem \ref{thm:MISP-main-theorem} we have guaranteed
