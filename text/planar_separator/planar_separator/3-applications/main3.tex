...
\emph{MISP} directly and have worst-case ratio and running time guaranteed by Lemma~
...
that even though \emph{MISP} algorithm was defined for problems with sets of vertices
it easily extendable to sets of edges which we can employ in this section. 
However it is worth noting that the \emph{MISP} algorithm is a subroutine of 
...
\begin{algorithmic}[1]
    \Procedure{MATCHING}{$G$, $M(G)$}
        ...
        \Else
        \State Run Algorithm \emph{MISP} with $\epsilon =
            {\log\log n}/N$ on $G$
...
\begin{enumerate}
    ...
    \item If $N \geq \log\log n$ and $\forall_{v \in V(G)} \, \deg(v) \geq 3$
        we apply \emph{MISP} algorithm. To prove its correctness we have to check conditions
...
\cite{gareyJohnsonStockmeyer1976}\cite{cormenIntroductionAlgorithms2022}. In
...
authors give an approximation algorithm (which is essentially \emph{MISP} algorithm discussed in Chapter 2) to find
...
\begin{algorithmic}[1]
    ...
    \While{\texttt{true}}
    \While{$\exists v \in V(G[V \setminus U])  \text{ with }  \deg(v) < 2$}
    \If{$\exists v \in V \setminus U$  \text{with}  $ \deg_{G[V \setminus U])}(v) = 0$}
        \State Add $v$ to $U$
    \EndIf
    \If{$\exists v \in V \setminus U$  \text{with}  $ \deg_{G[V \setminus U])}(v) = 1$ }
    ...
    \State $\forall$ $i = {1, 3, \dots, 2l + 1} \colon 0 \leq l < 
    \frac{k}{2}$ $\rightarrow$ add $v_i$ to $VC$
    \State $\forall$ $i = {1, \dots, k}$ $\rightarrow$ add $v_i$ to $U$
    \EndWhile
    \State $B(X, Y, E') \gets \text{BIPARTITE($G[V \setminus U]$)}$
...
The algorithm has three phases that can be extracted:
\begin{enumerate}
