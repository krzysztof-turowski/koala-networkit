...
number of pairwise nonadjacent edges in a graph. 

For bipartite graphs, one of the most known algorithms is the Hopcroft-Karp algorithm \cite{hopcroft1973n},
which finds the maximum matching in $O((m+n)\sqrt{n})$ time by repeatedly finding a maximal
set of vertex-disjoint shortest augmenting paths. For general graphs, the problem becomes
more difficult due to presence of odd cycles. Edmonds introduced the blossom algorithm \cite{edmonds1965paths},
which handles such cycles by contracting them during the search for augmenting paths and fins maximum
matchings in $O(n^3)$.
This approach was later improved by Micali and Vazirani, who developed an $O(m\sqrt{n})$ algorithm \cite{micali1980v}
for maximum matching in general graphs .

For planar graphs, we have already established that $m = O(n)$, so the best algorithms produce 
the maximum matching in $O(n^{3/2})$ time. A faster approach was proposed by Mucha and Sankowski 
who use an algebraic algorithm \cite{mucha2006maximum} based on Gaussian elimination that can compute the maximum 
matching in $O(n^{\omega/2})$ time, where $\omega$ is the exponent of matrix multiplication (as of 2026 
the best known bound is $\omega < 2.371177$ \cite{dupont2026improvingmatrixmultiplicationexponent}).

\subsection{Algorithm}
Although there exist
exact algorithms to find a maximum matching for planar graphs in $O(n \sqrt{n})$
...
One can find it interesting why we cannot use Algorithm
\textsc{MISP} directly and have worst-case ratio and running time guaranteed by Lemma~
...
the matching is a positive fraction of $n$.

It is also really important to see
that even though \textsc{MISP} algorithm was defined for problems with sets of vertices
it easily extendable to sets of edges which we can employ in this section. 
However it is worth noting that the \textsc{MISP} algorithm is a subroutine of 
Algorithm~\ref{alg:matching} ran on a reduced subgraph.

\begin{algorithm}[htbp]
\caption{\textsc{MATCHING} procedure}
\label{alg:matching}
\begin{algorithmic}[1]
    \Require $G \colon \text{planar graph}, M(G) \colon \text{matching}$
    \Ensure $M(G) \colon \text{matching}$
    \State $N \gets |V(G)|$
    \State $v \gets \min_{u \in V(G)} \deg(u)$
    \If{$N \leq \log\log n$}
        \State Find exact matching $M^*(G)$ using any reasonable algorithm
        \State $M(G) \gets M^*(G)$
    \ElsIf{$\deg(v) < 3$}
        \If{$\deg(v) = 0$}
            \State $G' \gets G - v$
            \State \Call{\textsc{MATCHING}}{$G', M(G')$}
            \State $M(G) \gets M(G')$
        \ElsIf{$\deg(v) = 1$}
            \State $u \gets \text{vertex adjacent to } v$
            \State $G' \gets G - \{u, v\}$
            \State \Call{\textsc{MATCHING}}{$G', M(G')$}
            \State $M(G) \gets M(G') \cup \{ (v, u) \}$
        \ElsIf{$\deg(v) = 2$}
            \State $u, w \gets \text{neighbors of } v$
            \State $G' \gets \text{graph made from } G \text{ by identifying }
            v, u \text{ and } w$
            \State \Call{\textsc{MATCHING}}{$G', M(G')$}
            \If{$M(G') \text{ does not have an edge adjacent to } u \text{
            in } G$}
                \State $M(G) \gets M(G') \cup \{(v, u)\}$
            \Else
                \State $M(G) \gets M(G') \cup \{(v, w)\}$
            \EndIf
        \EndIf

    \Else
    \State Run Algorithm \textsc{MISP} with $\epsilon =
        {\log\log n}/N$ on $G$
        \State $M(G) \gets \text{result of algorithm}$
    \EndIf
    \State \Return M
\end{algorithmic}
\end{algorithm}
...
\begin{theorem}\label{thm:matching}
    The approximation algorithm \textsc{MATCHING} for the maximum matching problem on
    planar graphs has running time $O(n \log n)$ and worst-case approximation ratio
...
two parts. First we will show that reductions performed by Algorithm~\ref{alg:matching}
...
worst-case ratio. We will define the maximum matching of $G$ as $M^*(G)$ and the
matching produced by algorithm as $M(G)$. We will show that
\begin{align*}
    \frac{|M(G)|}{|M^*(G)|} \geq 1 - O\left( \frac{1}{\sqrt{\log\log n}} \right)
\end{align*}

\subsubsection{Reductions}
\begin{enumerate}
    \item $G$ has a vertex of degree 0. Let $G' = G - v$. Then we have that
        $M(G) = M(G')$, as there is no edge adjacent to $v$. For the same reason
        we have $M^*(G) = M^*(G')$, so the reduction does not affect the ratio.
    \item $G$ has a vertex of degree 1. Let $u$ be the vertex adjacent to $v$ in
        $G$ and $G' = G - \{u, v\}$. If $M(G')$ is a matching produced by the
        algorithm then clearly $|M(G)| = |M(G')| + 1$ as we can add $(u,v)$ to
        the matching. Also since $M^*(G)$ has exactly one edge adjacent to $u$
        then by removing it we can reduce the matching by at most one, so we
        have $M^*(G) \leq M^*(G') + 1$. Now we can derive:
        $$
         \frac{|M(G)|}{|M^*(G)|} \geq
         \frac{|M(G')| + 1}{|M^*(G')| + 1} \geq
         \frac{|M(G')|}{|M^*(G')|}
        $$
    ...
    \item $G$ has a vertex of degree 2. Let $u, w$ be vertices adjacent to $v$
        ...
        vertices into single vertex $v'$. Let $M(G) = M(G') \cup \{(u,v)\}$ if no
        edge adjacent to $u$ in $G$ was selected to $M(G')$. Otherwise let
        $M(G) = M(G') \cup \{(w,v)\}$. Clearly we have $|M(G)| = |M(G')| + 1$. Now
        if the matching $M^*(G)$ contains either $(v,u)$ or $(v,w)$, let's say
        $(v,u)$ then by removing it by contraction to $v'$, $M^*(G')$ is a
        correct matching of size at most one less than $M^*(G)$. In the other
        case if both $u$ and $w$ were incident to edges in $M^*(G)$, then the
        matching $M^*(G')$ has to select one of them, so the property is
        conserved. In both cases we have $|M^*(G)| \leq |M^*(G')| + 1$. So again
...
\begin{enumerate}
    \item If $N \leq \log{\log{n}}$, so we reduced our graph to a small fraction
        ...
        $\frac{|M(G)|}{|M^*(G)|} = 1$ (if the denominator is 0, then numerator too, so by convention
        we set this ratio to 1)
    \item If $N \geq \log\log n$ and $\forall_{v \in V(G)} \, \deg(v) \geq 3$
        we apply \textsc{MISP} algorithm. To prove its correctness we have to check conditions
        from Lemma~\ref{lem:MISP-upperbound}:
        \begin{enumerate}
            \item Satisfied, as $M(G)$ is a matching so a subgraph induced by it
                satisfies the property that no two edges can have a common
                vertex
            \item Let $M^*(G)$ be the maximum matching. By removing vertices of
                a separator $C$ at most $|C|$ edges from the matching have
                incident vertices in $C$. So in the induced graph $G - C$ the
                maximum matching is of size at least $M^*(G) - |C|$. And this is
                also the size of $M(G)$ as we run the exact algorithm for
                matching here. So $|M(G)| \geq M^*(G) - |C|$, by rearranging we
                have $|M^*(G)| - |M(G)| \leq |C| = O(|C|)$, so the error is bound
                by size of $C$.
            \item Here we will use the theorem from \cite{nishizeki1979lower}
                ...
                at least 3, then $|M^*(G)| \geq N/3$, so the optimal solution is a
...
of higher. 
As each vertex is present in $O(\log{n})$ such rewirings, we get the
needed time complexity. To prove that, let's consider two vertices: $v$, $u$, 
such that $\deg(v) \geq \deg(u)$. It means that after rewiring edges from $u$ 
to $v$, we have $\deg_{1}(v) \geq \deg_{0}(u)$, where subscript denotes 
the number of iteration of rewirings. So after $k$ rewirings we have:
$\deg_k(v) = 2^k\deg(u)$, and because $2^k \leq n$, then $k \leq \log_2(n)$.
So each edge is rewired at most $O(\log{n})$ times.
\\
...
Another problem we will look at is Vertex Cover Problem which is NP-complete
even for planar graphs
\cite{gareyJohnsonStockmeyer1976, cormenIntroductionAlgorithms2022}. In
...
Vertex Cover Problem another interesting approach may be found in
\cite{bar-yehudaApproximatingVertexCover1982} which will be shown in this
section.
At first we shall define what a vertex cover is.
\begin{defn}[vertex cover, \cite{diestelGraphTheory2017}] \label{def:vertex-cover}
    Let $G = (V, E)$ be a graph. We call $VC \subseteq V$ a cover of $E$ (or 
    vertex cover of $G$) if every edge of $G$ is incident with a vertex in VC.
    This problem is $\mathbb{N}\mathbb{P}$-complete.
\end{defn}
Vertex Cover Problem asks for a minimum vertex cover in $G$. Another related problem is the
Independent Set Problem that asks for a maximum independent set.
\begin{defn}[independent set, \cite{diestelGraphTheory2017}]
    Let $G = (V, E)$ be a graph. We call $I \subseteq V$ and independent set if no 
    two of its elements are adjacent. This problem is $\mathbb{N}\mathbb{P}$-complete.
\end{defn}
It is clear that those problems are complementary, so solving Independent Set Problem
gives a solution to Vertex Cover Problem as $VC \cup I = V$. 
\subsection{Algorithm}
In \cite{liptonApplicationsPlanarSeparator1977}
authors give an approximation algorithm (which is essentially \textsc{MISP} algorithm discussed in Chapter 2) to find
...
\begin{defn}[rosary, \cite{bar-yehudaApproximatingVertexCover1982}] \label{def:rosary}
    Let $G = (V, E)$ be a simple planar graph. A \textbf{rosary} in $G$ is a cycle 
...
\end{defn}
\begin{defn}[bead, \cite{bar-yehudaApproximatingVertexCover1982}]  \label{def:bead}
Let $G = (V, E)$ be a simple planar graph. A vertex $v \in V$ of degree two is
called a \textbf{bead} if it plays the role of an even-indexed vertex $v_{2i}$ in
some rosary in $G$.
\end{defn}
\begin{defn}[proper graph, \cite{bar-yehudaApproximatingVertexCover1982}]
    Let $G = (V, E)$ be a simple planar graph. We will call it \textbf{proper} if the degree
    of all vertices is at least 2 and the set $Y_B$ of all its beads satisfies
$|Y_B| < 1/6 |V|$.
\end{defn}
\noindent
The authors prove a very useful
theorem regarding proper graphs.
\begin{theorem} \cite{bar-yehudaApproximatingVertexCover1982} \label{thm:proper}
...
\cite{bar-yehudaApproximatingVertexCover1982} has to reduce $G$ to a proper graph. The general
...
\begin{algorithmic}[1]
    ...
    \While{\texttt{true}}
    \While{\textbf{exists} $v \in V(G[V \setminus U])  \text{ with }  \deg(v) < 2$}
    \If{\textbf{exists} $v \in V \setminus U$  \text{with}  $ \deg_{G[V \setminus U])}(v) = 0$}
        \State Add $v$ to $U$
    \EndIf
    \If{\textbf{exists} $v \in V \setminus U$  \text{with}  $ \deg_{G[V \setminus U])}(v) = 1$ }
        \State Let $(v, u) \in E(G[V \setminus U])$ be the edge of $v$
        \State Add $v, u$ to $U$ and $u$ to $VC$
    \EndIf
    \EndWhile

    \While{there are simple isolated circuits in $G[V \setminus U]$}
    \State Let $v_1, \dots, v_k, v_1$ be a simple isolated circuit in $G[V \setminus U]$
    \For{$i = {1, \cdots, k}$}
    \If{$i$ is odd}
    \State add $v_i$ to $VC$
    \EndIf
    ...
    \EndFor
    \EndWhile
    \State $B(X, Y, E') \gets \text{\textsc{BIPARTITE}($G[V \setminus U]$)}$
    \If{$|Y| \geq \frac{1}{6} |V \setminus U|$}
        \State $VC \gets VC \cup X$
        \State $U \gets U \cup X \cup Y$
    \EndIf
    \If{$|Y| < \frac{1}{6} |V \setminus U|$ or $V \setminus U = \emptyset$}
        \State \texttt{break}
    \EndIf
    \EndWhile
    \EndProcedure
...
for $G[U]$ and an induced subgraph $G[V \setminus U]$ which shall be a proper graph.
The algorithm has three phases that can be extracted: deleting small degree vertices,
deleting isolated cycles, construction of an auxiliary bipartite graph.

The first phase is deleting all the vertices with vertex degree less than 2.
...
\end{itemize}
 ...
 The second phase is deleting all of the isolated cycles. It is easy to see that 
...
Now we shall focus on the third part of the algorithm, which is the construction of 
a bipartite graph from the residual graph $G[V \setminus U]$. The algorithm picks the whole
...
\begin{proof}
    ...
    so it is true for the original graph as well.

    If $\deg(y) = 1$, let $x$ by the adjacent vertex from $X$.
    ...
    By doing that we have that $X$ is the minimum vertex cover of the original graph.
...
\begin{algorithm}[H]
\caption{\textsc{BIPARTITE} procedure}
\label{alg:bipartite}
\small
\begin{algorithmic}[1]
    \Require $H \colon \text{planar graph}$
    \Ensure $B \colon \text{bipartite graph}$
    ...
    \ForAll{$v_L \in L$}
        \State label opposite to $v$ (using "+" or "-")
        \State $Q.add(v_L)$
    \EndFor
    \EndWhile
    ...
    \While{\textbf{exists} $v \in B: \deg_B(v) < 2$}
...
\end{algorithm}
        ...
        vertex cover of $B(X, Y, E)$. That is because each vertex with degree greater than 2 is labeled as $+$. This 
        opposite labelling construction serves as a natural separation of vertices into two sets, where after carefully
        selecting appropriate edges, we naturally get sets of independent vertices to construct a bipartite graph.
...
if the algorithm halts with the condition $|Y| < 1/6 |V \setminus U|$, then set $VC$ is the minimum vertex cover of $G[U]$.
...
\begin{proof}
    ...
    land in $Y$.
\end{proof}
By this lemma we have that $Y_B \subseteq Y$, so $G[V \setminus U]$ is a proper graph.

\subsection{Time complexity}
We can prove the following lemma.
\begin{lemma}
    Time complexity of the preparatory Algorithm~\ref{alg:prep} is $O(n)$.
\end{lemma}
\begin{proof}
 Construction of the 
