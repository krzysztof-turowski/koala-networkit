Let us introduce first the central concept of this work: a separator.
\begin{defn}
...
\begin{theorem} \cite{liptonSeparatorTheoremPlanar1979}  \label{thm:fnseparator}
    Let $\mathcal{S}$ be a class of graphs closed under a subgraph relation (i.e. if
    $G_1 \in \mathcal{S}$ and $G_2$ is a subgraph of $G_1$, then $G_2 \in \mathcal{S}$). There
    exist constants $\alpha < 1$, $\beta > 0$ such that if $G$ is any n-vertex
    graph in $\mathcal{S}$, the vertices of $G$ can be partitioned into three sets $A,B,C$
...
Such theorem for planar graphs, where $f(n) = O(\sqrt{n}),$ $ \alpha = 2/3,
$ $\beta = 1$ is called \textbf{planar separator theorem}.

First breakthrough in that area
...
What is really interesting about this theorem is that it holds even for 
weighted graphs, with weights (also called costs) assigned to vertices
However, at first we shall make a formal definition for that generalization.

\begin{defn} \label{defn:weighted}
    Weighted graph $G$ is a tuple $(V, E, w)$, where $V$ is called
    a vertex set, $E$ are 2-element subsets of $V$ (i. e. $(V, E)$ is a graph) and $w \colon V \rightarrow \mathbb{R}^+ \cup \{0\}$
...
graphs as well as some auxiliary lemmas.
...
\begin{defn}
    ...
    $t$ such that for all $v \in V$ it holds that $d_T(t, v) = d_G(t, v)$.
\end{defn}

Using such a spanning tree we can partition the vertex set $V$ into disjoint sets
...
\begin{proof}
    ...
    true. To prove that, let us consider $G'$ to be the path from $v$ to the root of $T$ and
    $C$ its vertex set.
    We can see that it
    has less than $2r + 1$ vertices and contains the root. Moreover: $W(C) \geq 1/3$
    so if we assign $A = V \setminus C$ and let $B
    = \emptyset$ then the proof is completed. 

    Thus let's assume that $G$ does not have a vertex with cost at least $1/3$.
    Let's now embed $G$ in plane using any algorithm (See discussion below
    \Cref{defn:embedding}),
    and make that graph maximal planar by 
    adding edges, so that each face is a triangle.

    Now each
    one of the non-tree edges, that is those added when making $G$ maximal as well,
...
\begin{proof}
    ...
    all vertices on levels $l_2$ and above, and contract levels $0, \dots,
    l_1$ into one vertex $t'$ with cost 0. From Lemma~\ref{lem:shrinking} we know that contracting
...
Following the constructions used in order to prove
...
\begin{enumerate}
    ...
    (see discussion following \Cref{defn:embedding}).
    ...
    \item Delete all vertices on level $l_2$ and above. Construct a new vertex
        ...
        \footnote{For details, see \cite{liptonSeparatorTheoremPlanar1979} or \texttt{LiptonTarjanPlanarSeparator.cpp}
        from Chapter 4.}. 
...
It is easy to see that all the steps of the algorithm except steps 7 and 8, take $O(n)$ time.
So let's prove that bound for latter as well.
\begin{lemma}
...
\end{proof}
    ...
    \begin{algorithmic}[1]
        \Require $C_1 \colon \text{cycle}$
        \Ensure $C \colon \text{fundamental cycle}$
        \State $i \gets 1$
        \State $C_i \gets C_1$
