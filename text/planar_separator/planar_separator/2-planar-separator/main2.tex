...
\section{Planar Separator Algorithm} \label{sec:planar-sep}
\input{2-planar-separator/main2_1.tex}

\section{Divide and Conquer Approach}
Let us recall the divide and conquer approach to solve problems
...
\begin{lemma} \cite{chibaApplicationsLiptonTarjans1981} \label{lem:epsilon-planar-separator}
	Let $G = (V, E, w)$ be a weighted planar graph with $n$ vertices, such that
    $W(G) \leq 1$ and let $0 < \epsilon < 1$. Then
...
\begin{proof}
	...
	\begin{algorithm}[H]\caption{Finding $\epsilon$-planar separator}\label{alg:epsilon-planar-separator}
		\begin{algorithmic}[1]
            \Require $G \colon \text{planar graph}$
            \Ensure $C \colon \text{separator}$
			\State $C \gets \emptyset$
			\While{$G - C$ has a connected component $K$ such that $W(K) > \epsilon$}
	...
	Let $L$ be a function that assigns a level to each
    ...
    With such a definition, we can see that for all $A, B$ it holds that 
    if $L(A) = L(B)$ then $A$ and $B$ are disjoint, because all components that are
	produced in the same iteration of the while loop are disjoint by construction of the algorithm and we can see that if $A$ is a subset of $B$, then $L(A) < L(B)$, because $A$ is produced by splitting $B$.
	...
	of cost before splitting.

    Now let's observe that because the cost of whole
...
\end{proof}
\noindent
By setting cost of each vertex in the graph to $1/n$ we obtain the following result:
\begin{corollary} \cite{chibaApplicationsLiptonTarjans1981} \label{cor:epsilon-planar-separator}
...
we can deduce an algorithm (Algorithm \ref{alg:MISP}, which we will call \textsc{MISP} from now on). We can clearly observe that the algorithm halts and successfully
solves the problem $P$.
\begin{algorithm}[H]\caption{\textsc{MISP} algorithm}\label{alg:MISP}
	\begin{algorithmic}[1]
        \Require $G \colon \text{planar graph}$, $\epsilon \in (0,1)$
        \Ensure $S \colon \text{subset of $V$ that induces a subgraph with property $Q$}$
        \State Apply Algorithm~\ref{alg:epsilon-planar-separator} to $G$  
		to find separator $C$
        \State Find connected components of $G - C$ with maximum size $\epsilon n$
    \ForAll{connected components $G_i$ of $G-C$}
		\State Find a maximum set $S_i$ inducing a subgraph
		with property $Q$ by checking every subset of vertex set $V$ for graph $G_i$
        \EndFor
...
\begin{lemma}\label{lem:MISP-upperbound} \cite{chibaApplicationsLiptonTarjans1981}
    The approximation \textsc{MISP} Algorithm (Algorithm~\ref{alg:MISP}) has time
...
\begin{proof}
    ...
    Algorithm \textsc{MISP} has two main steps that affect the time complexity. The
    first step is simply $O(n \log n)$ by Corollary
    \ref{cor:epsilon-planar-separator}. In step two, for each connected
    component $G_i = (V_i, E_i)$ we find $S_i$ by checking every subset. Let 
    $n_i = |V_i|$,
    ...
    $$
    O\left(\max \left\{ \sum_{i=1}^n n_i 2^{n_i} \colon \sum_{i=1}^n n_i = n \, \land \, 0 \leq n_i
...
$$
Which concludes the proof of Lemma~\ref{lem:MISP-upperbound}.
\end{proof}
