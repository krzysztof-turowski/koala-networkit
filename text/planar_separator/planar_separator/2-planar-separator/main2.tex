...
we can deduce an algorithm (Algorithm \ref{alg:MISP}, which we will call \emph{MISP} from now on). We can clearly observe that the algorithm halts and successfully
solves the problem $P$.
\begin{algorithm}[H]\caption{\emph{MISP} algorithm}\label{alg:MISP}
	\begin{algorithmic}[1]
        \Require $G$, $\epsilon \in (0,1)$
        \State Apply Algorithm~\ref{alg:epsilon-planar-separator} to a given graph
...
\begin{lemma}\label{lem:MISP-upperbound} \cite{chibaApplicationsLiptonTarjans1981}
    The approximation \emph{MISP} Algorithm (Algorithm~\ref{alg:MISP}) has time
...
\begin{proof}
    ...
    Algorithm \emph{MISP} has two main steps that affect the time complexity. The
