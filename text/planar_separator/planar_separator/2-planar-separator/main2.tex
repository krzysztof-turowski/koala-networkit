...
\begin{algorithm}[H]\caption{MISP algorithm}\label{alg:MISP}
	\begin{algorithmic}[1]
		...
		with property $Q$ by checking every subset of $V_i(G_i)$
...
\end{lemma}
\begin{proof}
	The proof follows proof in Lipton and Tarjan's book.
\end{proof}
...
\end{theorem}
\begin{proof}
	If we can prove that conditions from Lemma~\ref{lem:MISP-upperbound} are all
	satisfied, we can conclude the proof by setting $\epsilon = \log{\log{n}}/n$.
	Let's look at the conditions one by one:
	\begin{enumerate}
		\item beka
		\item this one is equivalent to $(c)$
	\end{enumerate}
\end{proof}
