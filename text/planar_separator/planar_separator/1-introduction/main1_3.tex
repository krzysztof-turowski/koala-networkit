...
\label{defn:spanning-tree}
    Let $G = (V, E)$ be a connected graph. We call $G' \subseteq G$ a spanning subgraph 
    of $G$ if $V' = V$.
\end{defn}
...
\begin{defn}[tree, \cite{diestelGraphTheory2017}]
\label{defn:tree}
...
\label{defn:bipartite}
    Let $G = (V, E)$ be a graph. We call $G$ a bipartite graph if there exists a partition of $V$ into two subsets $X, Y$ such that
...
\begin{defn}[isomorphism, \cite{diestelGraphTheory2017}]
\label{defn:isomorphism}
Let $G = (V, E)$ and $G' = (V', E')$. We call a map $\phi \colon V \rightarrow V'$ a homomorphism if $(\phi(u), \phi(v)) \in E'$ whenever $(u, v) \in E$. If $\phi$ is a bijection and $\phi^{-1}$ is also a homomorphism, then $\phi$ is an isomorphism. We call $G$ and $G'$ isomorphic if there exists a map $\phi \colon V \rightarrow V'$ that is an isomorphism.
\end{defn}
...
\begin{theorem} \cite[Euler's Formula]{diestelGraphTheory2017} \label{thm:euler}
...
\begin{lemma} \cite[maximal planar]{diestelGraphTheory2017} \label{lem:maximal}
...
\begin{theorem} \cite[Wagner's theorem]{diestelGraphTheory2017} \label{thm:wagner}
...
\label{defn:embedding}
    The planar embedding of a graph $G = (V, E)$ in a plane is an isomorphism
...
quickly, ideally in linear time. Luckily, there are several algorithms that do just that.
...
Because of \Cref{lem:small-deg} we can easily prove that for planar graphs 6 colors is enough. However, that is 
not the smallest number possible. In 1977 it was proven by Haken and Appel \cite{appel1977solution} that 
each planar graphs is four colourable, moreover it can be done in polynomial time. 
And although most of the $\mathbb{N}\mathbb{P}$-complete problems are still in this complexity class for planar graphs, there is another opportunity
