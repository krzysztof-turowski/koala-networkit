...
class MaximumWeightPerfectMatchingTest
    : public testing::TestWithParam<
        std::tuple<MaximumWeightMatchingParameters, MaxmimumWeightMatchingAlgorithm>> { };
...
void check_perfect_matching(const auto& matching) {
    for (auto [u, v] : matching) {
        EXPECT_NE(v, NetworKit::none);
    }
}
...
Koala::MaximumWeightMatching*
get_algorithm(MaxmimumWeightMatchingAlgorithm algorithm, NetworKit::Graph& G, bool perfect) {
    switch (algorithm) {
        case edmonds:
            return new Koala::EdmondsMaximumMatching(G, perfect);
        case gabow:
            return new Koala::GabowMaximumMatching(G, perfect);
        case micali:
...
TEST_P(MaximumWeightMatchingTest, test) {
    ...
    G.indexEdges(true);
    Koala::MaximumWeightMatching* algorithm = get_algorithm(matchingAlgorithm, G, false);
...
TEST_P(MaximumWeightPerfectMatchingTest, test) {
    auto const& [matchingParams, matchingAlgorithm] = GetParam();
    NetworKit::Graph G = build_weighted_graph(matchingParams.N, matchingParams.E);
    G.indexEdges(true);
    Koala::MaximumWeightMatching* algorithm = get_algorithm(matchingAlgorithm, G, true);

    algorithm->run();
    auto matching = algorithm->getMatching();

    check_proper_matching(G, matching);
    check_perfect_matching(matching);
...
INSTANTIATE_TEST_SUITE_P(
    test_example, MaximumWeightPerfectMatchingTest, testing::Combine(
        testing::Values(
            MaximumWeightMatchingParameters{4, {{0, 1, 3}, {1, 2, 10}, {2, 3, 3}}, 6},
            MaximumWeightMatchingParameters{4, {{2, 0, 8}, {3, 0, 6}, {3, 1, 10}, {3, 2, 10}}, 18},
            MaximumWeightMatchingParameters{
                10, {{1, 0, 2}, {2, 0, 8}, {2, 1, 10}, {4, 1, 2}, {5, 0, 5}, {5, 1, 9}, {5, 2, 8},
                     {6, 0, 5}, {6, 2, 8}, {6, 4, 8}, {7, 2, 2}, {7, 3, 4}, {7, 4, 8}, {7, 5, 4},
                     {8, 5, 4}, {8, 7, 5}, {9, 0, 10}, {9, 1, 4}, {9, 7, 6}, {9, 8, 1}}, 36},
            MaximumWeightMatchingParameters{
                6, {{2, 0, 7}, {2, 1, 1}, {3, 0, 19}, {3, 1, 18}, {3, 2, 3}, {4, 1, 18}, {4, 2, 3},
                    {5, 0, 20}, {5, 1, 20}, {5, 2, 9}, {5, 3, 7}, {5, 4, 13}}, 46},
            MaximumWeightMatchingParameters{
                8, {{1, 0, 9}, {2, 1, 5}, {3, 1, 6}, {3, 2, 9}, {4, 0, 6}, {4, 3, 4}, {5, 1, 5},
                    {6, 0, 1}, {6, 2, 3}, {7, 1, 3}, {7, 2, 7}, {7, 4, 3}}, 18},
            MaximumWeightMatchingParameters{
                8, {{2, 0, 8}, {2, 1, 4}, {3, 0, 8}, {3, 1, 4}, {3, 2, 7}, {4, 1, 2}, {4, 2, 9},
                    {5, 2, 2}, {5, 3, 2}, {5, 4, 8}, {6, 0, 4}, {6, 1, 8}, {6, 2, 10}, {6, 3, 4},
                    {6, 4, 2}, {6, 5, 6}, {7, 1, 1}, {7, 2, 3}, {7, 6, 8}}, 28},
            MaximumWeightMatchingParameters{
                12, {{0, 1, 14}, {0, 4, 14}, {0, 9, 7}, {1, 3, 14}, {2, 3, 14}, {2, 4, 14},
                     ...
                     {6, 8, 11}, {7, 8, 14}, {7, 10, 9}}, 62},
            MaximumWeightMatchingParameters{
                8, {{2, 0, 5}, {2, 1, 8}, {3, 1, 7}, {3, 2, 5}, {4, 1, 4}, {5, 0, 10}, {5, 2, 3},
                    {5, 3, 8}, {6, 0, 5}, {6, 1, 3}, {6, 2, 9}, {6, 5, 4}, {7, 1, 8}, {7, 2, 2},
                    {7, 5, 9}, {7, 6, 10}}, 29},
            MaximumWeightMatchingParameters{
                10, {{1, 0, 14}, {2, 0, 88}, {3, 0, 47}, {3, 1, 99}, {3, 2, 3}, {4, 0, 13},
                     {4, 1, 90}, {4, 3, 34}, {5, 4, 51}, {6, 0, 37}, {6, 1, 56}, {6, 2, 60},
                     {7, 0, 65}, {7, 1, 95}, {7, 2, 89}, {7, 3, 76}, {8, 6, 57}, {8, 7, 95},
                     {9, 2, 27}, {9, 6, 16}, {9, 7, 56}, {9, 8, 91}}, 367},
            MaximumWeightMatchingParameters{
                8, {{1, 0, 2}, {3, 0, 4}, {3, 1, 6}, {4, 1, 6}, {5, 0, 1}, {5, 4, 6}, {6, 0, 10},
                    {6, 3, 9}, {6, 4, 8}, {7, 2, 1}, {7, 4, 5}, {7, 5, 6}}, 23},
            MaximumWeightMatchingParameters{
                10, {{1, 0, 94}, {2, 0, 88}, {3, 0, 7}, {4, 1, 72}, {5, 0, 31}, {5, 2, 26},
                     {6, 5, 68}, {7, 0, 24}, {7, 2, 35}, {7, 6, 48}, {8, 7, 70}, {9, 2, 65},
                     {9, 3, 3}, {9, 4, 100}, {9, 5, 100}, {9, 6, 61}, {9, 8, 85}}, 301},
            MaximumWeightMatchingParameters{
                12, {{1, 0, 4}, {2, 1, 2}, {4, 0, 4}, {4, 1, 12}, {4, 2, 3}, {6, 1, 17}, {6, 4, 6},
                     {6, 5, 19}, {7, 3, 13}, {8, 5, 5}, {8, 7, 12}, {9, 0, 16}, {9, 3, 11},
                     {10, 1, 8}, {10, 2, 3}, {10, 4, 14}, {10, 5, 3}, {10, 9, 17}, {11, 0, 12},
                     {11, 3, 16}, {11, 4, 3}, {11, 6, 19}, {11, 7, 1}, {11, 10, 13}}, 79},
            MaximumWeightMatchingParameters{
                12, {{1, 0, 13}, {2, 0, 9}, {3, 0, 15}, {3, 2, 6}, {4, 0, 16}, {4, 2, 3}, {5, 1, 8},
                     {5, 3, 9}, {5, 4, 12}, {6, 2, 10}, {6, 3, 17}, {7, 1, 16}, {7, 2, 2},
                     {7, 5, 11}, {7, 6, 17}, {8, 1, 11}, {8, 2, 20}, {8, 3, 17}, {8, 5, 1},
                     {9, 1, 7}, {9, 2, 20}, {9, 4, 13}, {9, 5, 7}, {9, 6, 17}, {9, 8, 13},
                     {10, 1, 8}, {10, 3, 11}, {10, 6, 11}, {10, 7, 6}, {10, 8, 2}, {10, 9, 3},
                     {11, 0, 9}, {11, 1, 5}, {11, 2, 8}, {11, 3, 2}, {11, 4, 15}, {11, 5, 7},
                     {11, 8, 8}, {11, 9, 14}}, 88}),
        testing::Values(edmonds, gabow, micali, scaling)
));
